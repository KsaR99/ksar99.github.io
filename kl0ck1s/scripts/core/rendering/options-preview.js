...
export class OptionsPreviewController {
    ...
    _drawGridBackground(ctx, size, cache, colOffset, rowOffset, cols, rows) {
    ...
    _drawMainPanel(ctx, renderer, cache, size) {
        ...
        forEachShapeCell(mask, width, height, (r, c) => {
            const x = main.colOffset + (c - bounds.minX);
            const y = main.rowOffset + (r - bounds.minY);
            renderer.drawCell(ctx, x, y, color, size, {glow: true, level: levelFor(r - bounds.minY), cache});
        });

        this._drawGhostPiece(ctx, renderer, cache, {
            mask,
            width,
            height,
            color,
            bounds
        }, main.colOffset, main.rowOffset + ghostTopRow, pieceRows, size);
    }

    _drawTrailPanel(ctx, renderer, cache, size, now) {
        const {mask, width, height, color, bounds} = this._piece(renderer);
        const cols = bounds.width;
    ...
    _drawFlashPanel(ctx, renderer, cache, size, now) {
        const {mask, width, height, color, bounds} = this._piece(renderer);
        const cols = bounds.width;
        const pieceRows = bounds.height;
    ...
    _drawGhostPiece(ctx, renderer, cache, piece, colOffset, ghostTopRow, pieceRows, size) {
        if (renderer.ghostType === "off") return;

        const {mask, width, height, color, bounds} = piece;
        const applyOpacity = renderer.transparencyEnabled;

        if (renderer.ghostType === "radioactive") {
            const sprite = cache.getOutlineGhost(color, size, 0);
            const offset = (sprite.width - size) / 2;
            ctx.save();
            if (applyOpacity) ctx.globalAlpha *= renderer.ghostOpacity;
            forEachShapeCell(mask, width, height, (r, c) => {
        ...
        const baseColor = renderer.ghostType === "white" ? GHOST_WHITE_COLOR : color;
        const lightColor = renderer.ghostType === "white" ? GHOST_WHITE_COLOR : lightenOklch(baseColor);
        const fillColor = applyOpacity ? baseColor : lightColor;

        if (applyOpacity) ctx.save();
        if (applyOpacity) ctx.globalAlpha *= renderer.ghostOpacity;
        forEachShapeCell(mask, width, height, (r, c) => {
            const x = colOffset + (c - bounds.minX);
            const localY = r - bounds.minY;
            const level = renderer.heightSaturationEnabled ? Math.max(0, pieceRows - 1 - localY) : 0;
            renderer.drawCell(ctx, x, ghostTopRow + localY, fillColor, size, {level, cache});
        });
        ...
        forEachShapeCell(mask, width, height, (r, c) => {
            ...
            const level = renderer.heightSaturationEnabled ? Math.max(0, pieceRows - 1 - localY) : 0;
            ctx.strokeStyle = withAlpha(colorForLevel(baseColor, level), 0.6);
            ctx.beginPath();
