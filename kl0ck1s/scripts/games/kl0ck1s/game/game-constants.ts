...
function invertKicks(kicks: readonly Kick[]): Kick[] {
    return kicks.map(([dx, dy]) => [-dx, -dy]);
}

function withReverse(cwTable: KickTable): KickTable {
...
export const NICKNAME_PATTERN = /^[\p{L}\p{N}_-]{3,16}$/u;
...
export const SQUARE_SPIN_SCALE_DIP = 0.25;

export function fallTrailLengthForInterval(dropIntervalMs: number): number {
