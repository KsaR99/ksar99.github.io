...
import {defaultKeyBindings, formatKeyCode, KEY_BIND_SLOTS, type KeyBindingMap} from "../shared/key-bindings.js";
import type {I18n} from "../services/i18n.js";

const ALL_VOICE_NUMBER_KEYS = new Set([...VOICE_COUNTING_KEYS_ALL, ...VOICE_ORDINAL_KEYS_ALL]);

function setMuteToggleState(button: HTMLElement | null, muted: boolean, i18n: I18n, effectiveMuted = muted): void {
    if (!button) return;
...
function clone(dom: Document, templateId: string): DocumentFragment {
    return dom.getElementById(templateId).content.cloneNode(true);
}

function fillDifficultyCarousel(container: Element | null, selectedDifficulty: string, difficulties: Record<string, {
    startLevel: number
}>, i18n: I18n): void {
    if (!container) return;
...
function fillModeCarousel(container: Element | null, selectedMode: string, i18n: I18n): void {
    if (!container) return;
    container.querySelector('[data-field="modeLabel"]').textContent = i18n.t(`modes.${selectedMode}.name`);
}

function fillModeDescription(container: Element | null, selectedMode: string, i18n: I18n): void {
    if (!container) return;
    container.textContent = `💡 ${i18n.t(`modes.${selectedMode}.description`)}`;
}

function fillProfileSelect(dom: Document, select: HTMLSelectElement | null, profiles: string[], current: string, i18n: I18n, trash: Array<{
    name: string
}> = []): void {
    if (!select) return;
    select.innerHTML = "";
...
function fillSoundRows(dom: Document, container: Element, keys: string[], soundVolumes: Record<string, number>, soundMuted: Record<string, boolean>, i18n: I18n): void {
    keys.forEach((key) => {
...
function groupedKeyBindSlots(): Array<{ labelKey: string; slots: typeof KEY_BIND_SLOTS[number][] }> {
    const groups = [];
...
function renderKeybindRows(dom: Document, container: Element, keyBindings: KeyBindingMap | null | undefined, i18n: I18n): void {
    container.innerHTML = "";
...
const DIFF_LABEL_KEYS = {
    ...
    showFirstGameTutorial: "screens.options.showFirstGameTutorial",
...
const BENCHMARK_LABEL_KEYS = {
    ...
    renderWebGL: "benchmark.categories.renderWebGL",
...
const GHOST_TYPE_LABEL_KEYS = {
    ...
    ascii: "screens.options.ghostTypeAscii",
...
function formatSettingValue(key: string, value: unknown, i18n: I18n): string {
    ...
    if (key === "touchSensitivity") {
        ...
        return n === 1 ? i18n.t("screens.options.autoValue") : `${Math.round(n * 100)}%`;
    }
    if ((key === "volume" || key === "mouseSensitivity") && typeof value === "number") return `${Math.round(value * 100)}%`;
    if ((key === "keyboardDAS" || key === "keyboardARR") && typeof value === "number") return `${Math.round(value)} ms`;
    if (key === "outlineBlocks") {
        return i18n.t(value ? "screens.options.blockTypeRadioactive" : "screens.options.blockTypeColorful");
    }
    if (key === "ghostType" && typeof value === "string") return i18n.t(GHOST_TYPE_LABEL_KEYS[value] ?? "screens.options.ghostTypeColorful");
...
function fillModeInfoRules(dom: Document, container: Element, mode: string, i18n: I18n): void {
...
const asElement = <T extends Element>(element: Element | null): T => {
    if (!element) throw new Error("Expected screen element");
    return element as T;
};
...
export const Screens = {
    ...
    loading(title: string, text: string, dom: Document = document) {
    ...
    modeChoice(dom: Document = document, i18n: I18n) {
        const screen = clone(dom, "tpl-screen-mode-choice");
        i18n.applyStatic(screen);
        return screen;
    },

    idle(list: unknown[], selectedDifficulty: string, difficulties: Record<string, {
        startLevel: number
    }>, selectedMode: string, gameModes: Record<string, {
        noLeaderboard?: boolean
    }>, renderLeaderboard: (list: unknown[]) => Node, dom: Document = document, i18n: I18n, playerName = "", profiles: string[] = [], trash: Array<{
        name: string
    }> = []) {
        ...
        asElement<HTMLInputElement>(screen.querySelector('[data-role="name-input"]')).value = playerName;
        fillProfileSelect(dom, screen.querySelector('[data-role="profile-select"]'), profiles, playerName, i18n, trash);
        asElement<HTMLButtonElement>(screen.querySelector('[data-role="delete-profile-button"]')).disabled = !playerName;
    ...
    options(settings: Record<string, any>, dom: Document = document, i18n: I18n, soundManager: {
        keysInCategory(category: string): string[]
    } | null = null, context = "options") {
        ...
        const optionsMuteToggle = asElement<HTMLButtonElement>(screen.querySelector('[data-role="options-mute-toggle"]'));
        const volumeSlider = asElement<HTMLInputElement>(screen.querySelector('[data-role="volume-slider"]'));
        setMuteToggleState(
        ...
        asElement<HTMLInputElement>(screen.querySelector('[data-role="hud-right-checkbox"]')).checked = settings.hudRight;
        ...
        const ghostTypeSelect = asElement<HTMLSelectElement>(screen.querySelector('[data-role="ghost-type-select"]'));
        if (ghostTypeSelect) ghostTypeSelect.value = settings.ghostType ?? "white";
        const ghostOpacitySlider = asElement<HTMLInputElement>(screen.querySelector('[data-role="ghost-opacity-slider"]'));
        ...
        asElement<HTMLInputElement>(screen.querySelector('[data-role="grid-checkbox"]')).checked = settings.gridLines;
        ...
        if (blockTypeSelect) blockTypeSelect.value = settings.blockType ?? (settings.asciiFallingPieces ? "ascii" : (settings.outlineBlocks ? "radioactive" : "colorful"));
        const activeBlockType = settings.blockType ?? (settings.asciiFallingPieces ? "ascii" : (settings.outlineBlocks ? "radioactive" : "colorful"));
        const isColorfulBlockType = activeBlockType === "colorful";
        if (!isColorfulBlockType) settings.heightSaturation = false;
        const heightSaturationCheckbox = screen.querySelector('[data-role="height-saturation-checkbox"]');
        if (heightSaturationCheckbox) {
            heightSaturationCheckbox.checked = isColorfulBlockType && Boolean(settings.heightSaturation);
            heightSaturationCheckbox.disabled = !isColorfulBlockType;
        }
        const heightSaturationRow = screen.querySelector('[data-role="height-saturation-row"]');
        if (heightSaturationRow) heightSaturationRow.hidden = !isColorfulBlockType;
        asElement<HTMLInputElement>(screen.querySelector('[data-role="glow-checkbox"]')).checked = settings.glow;
        asElement<HTMLInputElement>(screen.querySelector('[data-role="transparency-checkbox"]')).checked = settings.transparency;
        ...
        const showFirstGameTutorialCheckbox = screen.querySelector('[data-role="show-first-game-tutorial-checkbox"]');
        if (showFirstGameTutorialCheckbox) showFirstGameTutorialCheckbox.checked = settings.showFirstGameTutorial !== false;
        ...
        const langSelect = asElement<HTMLSelectElement>(screen.querySelector('[data-role="lang-select"]'));
        if (langSelect) {
    ...
    renderImportDiffRows(dom: Document, container: Element, changes: Array<{
        key: string;
        oldValue: unknown;
        newValue: unknown
    }>, i18n: I18n): void {
    ...
    renderBenchmarkResults(dom: Document, container: Element, results: Array<{
        key: string;
        totalMs: number;
        percent: number;
        avgMs: number
    ...
    formatBenchmarkResultsText(results: Array<{
        key: string;
        totalMs: number;
        percent: number;
        avgMs: number
    }>, i18n: I18n, {pieceCount, totalMs}: { pieceCount?: number; totalMs?: number } = {}): string {
        const lines = [];
    ...
    modeInfo(mode: string, dom: Document = document, i18n: I18n) {
    ...
    countdown(number: string | number, tint: string | null, dom: Document = document) {
    ...
    gameOverEntry(stats: any, list: unknown[], highlightEntry: any, todayBestEntry: any, renderLeaderboard: (list: unknown[]) => Node, dom: Document = document, i18n: I18n, reason = "topOut") {
