...
export class StatsTracker {
    ...
    reset() {
        const service = this.game.scoringService;
        if (service) {
    ...
    registerPieceSpawn(type) {
        const service = this.game.scoringService;
        if (service) {
            service.registerPieceSpawn(type);
            return;
        }
        const game = this.game;
        ++game.piecesSpawned;
    ...
    registerLineClears(cleared, playSound = true) {
        const service = this.game.scoringService;
        if (service) {
            service.registerLineClears(cleared, playSound);
            return;
        }
        const game = this.game;
        if (cleared === 0) return;
    ...
    registerSpin(spin, cleared) {
        const service = this.game.scoringService;
        if (service) {
            service.registerSpin(spin, cleared);
            return;
        }
        const game = this.game;
        if (spin.type === "T") {
            if (spin.mini) ++game.spinCounts.tMini; else ++game.spinCounts.t;
        } else {
