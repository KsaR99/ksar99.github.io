...
export interface GameSettings {
    ...
    asciiFallingPieces: boolean;
    outlineBlocks: boolean;
    blockType: "radioactive" | "colorful" | "ascii";
...
export class SettingsController {
    ...
    defaultSettings(): GameSettings {
        return {
            ...
            fallTrail: true,
            hardDropFlash: true,
            asciiFallingPieces: false,
            outlineBlocks: true,
            blockType: "radioactive",
            categoryVolumes: {
                sfx: 1,
                music: 0.1,
                voices: 0.5
    ...
    applyStoredSettings(stored: Partial<GameSettings> & Record<string, unknown>): void {
        const game = this.game;
        const defaults = this.defaultSettings();
        const settings = {...defaults, ...stored};
        if (stored.blockType === undefined) {
            settings.blockType = stored.asciiFallingPieces
                ? "ascii"
                : (stored.outlineBlocks ? "radioactive" : "colorful");
        }
        if (!["radioactive", "colorful", "ascii"].includes(settings.blockType)) settings.blockType = "radioactive";
        settings.outlineBlocks = settings.blockType === "radioactive";
        settings.asciiFallingPieces = settings.blockType === "ascii";
    ...
    applyPerformanceSettings() {
        ...
        game.renderer.setAsciiFallingPiecesEnabled?.(blockType === "ascii");
