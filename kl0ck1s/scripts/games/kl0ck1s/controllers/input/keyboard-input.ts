...
export class KeyboardInput extends InputSource {
    ...
    _keyupHandler: null;
    _blurHandler: (() => void) | null;
    _listening: false | true;
    _listenHandler: null;
    _heldHorizontal: Set<string>;
    ...
    bind() {
        ...
        this._keydownHandler = (event) => {
            ...
            if (game.multiplayerController?.isOpen && game.state === "idle") return;
            ...
            if (isMenuScreen && (event.code === "ArrowLeft" || event.code === "ArrowRight") && game.menuSelector === "entry") {
                event.preventDefault();
                if (!event.repeat) game.screenFlow.moveMenuFocus(event.code === "ArrowRight" ? 1 : -1, "horizontal");
                return;
            }
