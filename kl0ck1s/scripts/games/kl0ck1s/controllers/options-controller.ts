...
import type {GameFlowController} from "./game-flow-controller.js";
import {
    ARR_MAX,
    ARR_MIN,
    ARR_STEP,
...
type OptionsControlElement = HTMLElement & {
    value: string;
    checked: boolean;
    disabled: boolean;
    hidden: boolean;
...
function queryControl(root: ParentNode, selector: string): OptionsControlElement | null {
    return root.querySelector(selector) as OptionsControlElement | null;
}
...
export class OptionsController {
    ...
    _pauseBlockedTimer: null | number;
    graphicsPreview: OptionsPreviewController;
    dasArrPreview: DasArrPreviewController;
    optionsSearchQuery: string;
    pendingImportChanges: ImportReviewChange[] | null;
    dataTransfer: OptionsDataTransferController;

    constructor(game: Game, gameFlow: GameFlowController) {
        this.game = game;
    ...
    toggleMultiplayerLiveOptions() {
        ...
        game.multiplayerController?.setOpponentPausedVisual(true);
    ...
    renderPauseMenu() {
        const game = this.game;
        game.hud.showScreen(game.screens.options(game.settings, game.dom ?? undefined, game.i18n, game.soundManager, "pause"));
    ...
    bindLangSelect() {
        ...
        const select = queryControl(game.hud.overlayEl, '[data-role="lang-select"]');
        if (!select) return;
    ...
    renderOptionsMenu() {
        ...
        game.hud.showScreen(game.screens.options(game.settings, game.dom ?? undefined, game.i18n, game.soundManager));
        this.bindOptionsMenu();
    ...
    togglePreviewButton(button: OptionsControlElement, list: Element) {
        const game = this.game;
        ...
        list.querySelectorAll<HTMLElement>('[data-role="sound-preview-button"]').forEach((otherButton) => {
            if (otherButton !== button) this.setPreviewButtonState(otherButton as OptionsControlElement, "play");
        });
    ...
    setPreviewButtonState(button: OptionsControlElement, state: "play" | "pause") {
        const game = this.game;
    ...
    bindOptionsMenu() {
        ...
        const optionsMuteToggle = queryControl(root, '[data-role="options-mute-toggle"]');
        const volumeSlider = queryControl(root, '[data-role="volume-slider"]');
        const hudRightCheckbox = queryControl(root, '[data-role="hud-right-checkbox"]');
        const ghostTypeSelect = queryControl(root, '[data-role="ghost-type-select"]');
        const ghostOpacitySlider = queryControl(root, '[data-role="ghost-opacity-slider"]');
        const ghostOpacityRow = queryControl(root, '[data-role="ghost-opacity-row"]');
        const gridCheckbox = queryControl(root, '[data-role="grid-checkbox"]');
        const screenShakeCheckbox = queryControl(root, '[data-role="screen-shake-checkbox"]');
        const heightSaturationCheckbox = queryControl(root, '[data-role="height-saturation-checkbox"]');
        const glowCheckbox = queryControl(root, '[data-role="glow-checkbox"]');
        const transparencyCheckbox = queryControl(root, '[data-role="transparency-checkbox"]');
        const fallTrailCheckbox = queryControl(root, '[data-role="fall-trail-checkbox"]');
        const hardDropFlashCheckbox = queryControl(root, '[data-role="hard-drop-flash-checkbox"]');
        const blockTypeSelect = queryControl(root, '[data-role="block-type-select"]');
        const themeGrid = queryControl(root, '[data-role="theme-grid"]');
        const skipCountdownCheckbox = queryControl(root, '[data-role="skip-countdown-checkbox"]');
        const showFirstGameTutorialCheckbox = queryControl(root, '[data-role="show-first-game-tutorial-checkbox"]');
        const mouseControlCheckbox = queryControl(root, '[data-role="mouse-control-checkbox"]');
        const mouseSensitivityInput = queryControl(root, '[data-role="mouse-sensitivity-input"]');
        const touchSensitivityInput = queryControl(root, '[data-role="touch-sensitivity-input"]');
        const keyboardDasInput = queryControl(root, '[data-role="keyboard-das-input"]');
        const keyboardArrInput = queryControl(root, '[data-role="keyboard-arr-input"]');
        const closeButton = queryControl(root, '[data-role="options-close-button"]');
        const closeKey = queryControl(root, '[data-role="options-close-key"]');
        ...
        const heightSaturationRow = queryControl(root, '[data-role="height-saturation-row"]');
        ...
        if (themeGrid) {
            themeGrid.addEventListener("click", (event: Event) => {
                const card = (event.target as Element | null)?.closest<HTMLElement>('[data-role="theme-option"]');
                if (!card) return;
        ...
        const skipModeInfoCheckbox = queryControl(root, '[data-role="skip-mode-info-checkbox"]');
        ...
        root.querySelectorAll('[data-role="sound-list"], [data-role="sound-list-countdown"]').forEach((list) => {
            list.addEventListener("input", (event: Event) => {
    ...
    categoryResetGroups() {
        const graphicsKeys = ["screenShake", "gridLines", "ghostType", "ghostOpacity", "blockType", "heightSaturation", "glow", "transparency", "fallTrail", "hardDropFlash"];
        return {
    ...
    setImportReviewVisible(visible: boolean) {
        this.dataTransfer.setImportReviewVisible(visible);
    }

    showImportMessage(kind: "empty" | "invalid") {
        this.dataTransfer.showImportMessage(kind);
    }

    showImportReview(changes: ImportReviewChange[]) {
        this.dataTransfer.showImportReview(changes);
    }
