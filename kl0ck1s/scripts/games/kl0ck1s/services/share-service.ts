...
function canvasSupportsOklch(dom: Document): boolean {
...
function angleGradientPoints(angleDeg: number, x: number, y: number, w: number, h: number): {
    x1: number;
    y1: number;
    x2: number;
    y2: number
...
export class ShareService {
    ...
    _themeColor(name: keyof typeof THEME_COLOR_FALLBACKS): string {
        const dom = this.dom;
    ...
    _formatCardDate(iso: string): string {
    ...
    async shareLeaderboard(mode: string) {
        const game = this.game;
    ...
    bindIconButton(button: HTMLButtonElement | null, action: () => Promise<{ status: string }>): void {
        if (!button) return;
    ...
    bindLabeledButton(button: HTMLButtonElement | null, action: () => Promise<{ status: string }>): void {
        if (!button) return;
    ...
    _drawLogo(ctx: CanvasRenderingContext2D, centerX: number, y: number): void {
        const text = APP_NAME;
        const fontSize = 58;
    ...
    _drawChips(ctx: CanvasRenderingContext2D, chips: Array<{
                   label: string;
                   value: string
               }>, centerX: number, y: number,
               {panel, line, muted, text}: { panel: string; line: string; muted: string; text: string }): void {
        const height = 96;
        const gap = 18;
    ...
    _fitText(ctx: CanvasRenderingContext2D, text: string, maxWidth: number): string {
    ...
    _shareText(modeLabel: string, score: number, name: string | null = null): string {
        const i18n = this.game.i18n;
        return name
    ...
    _tableShareText(modeLabel: string): string {
        return this.game.i18n.t("share.tableText", {app: APP_NAME, mode: modeLabel});
    }

    _statusIcon(status: string): string {
    ...
    async _dispatch(canvas: HTMLCanvasElement, {text, filename}: { text: string; filename: string }): Promise<{
        status: string
    }> {
    ...
    _download(blob: Blob, filename: string): void {
        const dom = this.dom;
